...
- CLAUDE.md 200行以内。30-100行が最高効率帯。
...
## Phase 1: インタビュー（15-40問）
4段階ファネル。1ターン最大3問。選択肢付き優先。
...
Hooks：PostToolUse で Write|Edit 時に prettier 自動適用を設定する。
...
ステップ7: claude mcp add playwright -s project -- npx -y @playwright/mcp@latest
...
## Phase 3: 品質チェック
ナレッジのセクション[J]チェックリスト19項目を全確認。1つでも不合格なら修正。
